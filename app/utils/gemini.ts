...
const diagnosticPrompt = `Create a 10-question proficiency diagnostic test for ${targetLanguage} for a speaker of ${nativeLanguage}.
The test MUST include exactly:
- 2 questions at A1 level (Basic survival phrases, simple pronouns)
- 2 questions at A2 level (Past tense, basic conjunctions, frequent topics)
- 2 questions at B1 level (Experiences, dreams, hopes, reasons/explanations)
- 2 questions at B2 level (Abstract topics, technical discussion, fluent interaction)
- 2 questions at C1 level (Implicit meaning, flexible use for social/professional purposes)

CRITICAL QUALITY RULES:
1. TEST GRAMMAR & CONTEXT: Focus heavily on grammar patterns, particles (like Japanese は/が/を), and word choice nuances rather than just vocabulary.
2. NO VISUAL CHEATS: Ensure the answer cannot be guessed by matching symbols/Kanji from the question text to the options. The target word MUST be completely hidden by "____".
3. DISTRACTOR PLAUSIBILITY: Distractors must be grammatically correct in some contexts but wrong for THIS specific sentence. Avoid obviously wrong answers.
4. SYSTEM FORMAT: Use the exact JSON format defined in your instructions.
5. LANGUAGE: Questions in ${targetLanguage}, explanations in ${nativeLanguage}.`;
